...
require_once dirname(__FILE__) . DS . 'gcurl.php';

class ogb_htmlparser_code extends ogb_htmlparser_get_CURL {
	...
	public static function runFuncs( $funcs, $rows ) {
		...
		switch ( $funcs[0] ) {
			...
			case 'replace':
				...
				$res       = call_user_func_array( array( 'ogb_htmlparser_code', 'replace' ), $param_arr );
	...
	public static function replace( $inputs, $search, $replace, $reg = 0, $reg_limit = 0, $reg_pattern = '' ) {
		$search = preg_replace( '/(\\\){2,}/i', '$1', $search );
		$arg = func_get_args();
		if ( $reg ) {
			if ( $reg_limit ) {
